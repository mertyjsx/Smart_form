...
useEffect(() => {
 ...
 set_awards(old.award)
    ...
    const deleteCourse = (course) => {
        let course_array = courses.filter(item => item.id !== course.id)
    ...
    const  deleteAward = (award) => {
        let award_array = awards.filter(item=> item.id !== award.id)
     ...
     const deleteField = (field) => {
        ...
        let field_array = fields.filter(item => item.id !== field.id)
    ...
    set_fields(field_array)
     ...
     const deleteHonor = (honor) => {
        ...
        let honor_array = honors.filter(item => item.id !== honor.id)
...
const calculate=(name)=>{
    ...
    let isResume=resume&&resume.find(name)
    let isPublic=pub&&pub.find(name)
    let isPrivate=pri&&pri.find(name)
...
<h5 className="m-30 color p-12">Study of field</h5>
 {fields&&fields.map(item=><OldComponent old={item} del={deleteField} edit={editField} name="field_of_study" label="field of study" handleCheckbox={handleCheckbox} pub={pub} pri={pri} resume={resume}></OldComponent>)}
</Paper>
...
<h5 className="m-30 color p-12">Honors</h5>
 ...
 {honors&&honors.map(item=><OldComponent old={item} del={deleteHonor} edit={editHonor} label="honor" name="honor" handleCheckbox={handleCheckbox} pub pri resume></OldComponent>)}
  
</Paper>
<Paper elevation={3} className="paper-container">
<h5 className="m-30 color p-12">Courses</h5>
{courses&&courses.map(item=><OldComponent old={item} del={deleteCourse} label="course" edit={editCourse} name="course" handleCheckbox={handleCheckbox} pub pri resume></OldComponent>)}
</Paper>
<Paper elevation={3} className="paper-container">
<h5 className="m-30 color p-12">Awards</h5>
{awards&&awards.map(item=><OldComponent old={item} del={deleteAward} label="award" edit={editAward} name="award" handleCheckbox={handleCheckbox} pub pri resume></OldComponent>)}
</Paper>
