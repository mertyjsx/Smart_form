...
export default function OldComponent({old,edit,handleCheckbox,del,name,pub,pri,resume,label}) {
  ...
  <Grid container>
    ...
    <Grid container xs={12} md={7} className={`p-12 `} alignItems="center" alignContent="center" >
      <FormControl autoComplete="off" className="fullw">
        ...
        <TextField disabled={!enabled} defaultValue={old.value} className="fullw" onChange={handleChange} id="outlined-basic" name={name} label={label} variant="outlined" required />
      </FormControl>
    </Grid>
    <Grid item xs={12} md={5} className="p-12">
      <FormControl variant="outlined" className="rightselect" >
        <MultipleSelect
defaultValue={calculate(name)}
